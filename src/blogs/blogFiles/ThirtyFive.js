            ...
            <Grid align="left" width="75%">
                <p>
                    ...
                    <ul>
                        ...
                        <li><pre>{`db.articles.inserOne({"title":"mongoDb", "level":"basic"})`}</pre>
                        ...
                        <li>we can add id manually, but it should be unique. If it's not unique, db will throw an error.
                            <pre>{`db.articles.inserOne({"_id":"abc","title":"mongoDb", "level":"basic"})`}</pre>
                        </li>
                        ...
                        <li><pre>{`db.articles.find({}).forEach((article) => {printjson(article.author)}))`}</pre></li>
                        <li><pre>{`db.articles.find({},{title:1})`}</pre> // it will give only id and title column. 1 denotes show the column.</li>
                        <li><pre>{`db.articles.find({},{title:1, "_id": 0})`}</pre> // it will give only title column. 0 denotes don't shoe the column.</li>
                        <li><pre>{`db.articles.find({})`}</pre></li>
                        ...
                        <li>
                            <pre>{`db.articles.deleteOne({"_id":"abc"})`}</pre> // it deletes the record where id is abc.
                        </li>
                        <li>
                            <pre>{`db.articles.deleteMany({"level":"basic"})`}</pre> // it deletes all the records where level is basic.
                        </li>
                        <li>
                            <pre>{`db.articles.updateOne({"_id":"abc"}, {$set : {"author": "Dark Lord"}})`}</pre> // it updates one record where id is abc and adds author field. "set" is a keyword, hence we need to put $ in front of it.
                        </li>
                        <li>
                            <pre>{`db.articles.update({"_id":"abc"}, {$set : {"author": "Dark Lord"}})`}</pre> // it overrides one record where id is abc and adds author field.
                        </li>
                        <li>
                            <pre>{`db.articles.updateMany({"level":"basic"}, {$set : {"author": "Dark Lord"}})`}</pre> // it updates all the records where level is basic.
                        </li>
